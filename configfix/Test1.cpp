...
typedef struct Location {
  ...
  Location() : autoindex(off) {}  // Constructor
} Location_t;
...
void setLocationRoot(Location& loc, const std::string& root) {
  loc.root = root;
}

void addLocationMethod(Location& loc, const std::string& method) {
  loc.allowedMethods.push_back(method);
}

void setLocationAutoindex(Location& loc, bool autoindex) {
  loc.autoindex = autoindex;
}

void addLocationIndexFile(Location& loc, const std::string& file) {
  loc.indexFiles.push_back(file);
}

void setLocationUrl(Location& loc, const std::string& url) { loc.url = url; }
...
void addServConfigLocation(ServConfig& serv, const Location& loc) {
...
void Server::print() const {
  for (size_t i = 0; i < servConfigs.size(); ++i) {
    ...
    for (const Location& loc : config.locations) {
      std::cout << "Location URL: " << loc.url << "\n"
